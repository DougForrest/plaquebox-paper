...
export INSTANCE_NAME="plaquebox-paper-replication"
...
```bash
# Preprocessing machine for paper replication
export INSTANCE_NAME="plaquebox-paper-replication"
...
# budget --accelerator="type=nvidia-tesla-t4,count=1"
# highend --accelerator="type=nvidia-tesla-v100,count=1"
gcloud compute instances create $INSTANCE_NAME \
		--project=$PROJECT_ID \
        --zone=$ZONE \
        --image-family=$IMAGE_FAMILY \
